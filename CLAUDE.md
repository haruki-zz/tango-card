...
- src/main/ipcHandlers.ts：主进程注册白名单 IPC handler，AI 通道委托 AiClient，DB 通道使用 reviewQueueService、reviewService、activityService 提供复习与活跃度能力，支持注入自定义 aiClient 与 database。
...
- src/renderer/App.tsx：渲染新增、复习与 Heat Map 视图的壳组件。
...
- tests/ipc-boundary.test.ts：验证 createPreloadApi 与主进程 handler 的通路，AI 通道通过 stubbed AiClient，确保 window.api 方法可用，并覆盖 Heat Map 通道。
...
- 主进程 handler 已接入真实 DB 写入（新增、复习、活跃度聚合），AI 仍可注入 stub；其他通道可逐步替换为真实实现。
- 渲染层完全基于 React/Vite，包含新增、复习与 Heat Map 展示，样式与状态管理将继续在此扩展。
