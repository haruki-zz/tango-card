...
- AI：可替换的 OpenAI 兼容 HTTP 接口，封装为主进程服务，经安全 IPC 调用。

## 核心功能
1) **新增单词（AI 丰富）**
   - 输入日语单词；可选手动补充释义/例句。
   - AI 返回：罗马音、带情景的简洁释义、1–2 组例句（JP + EN gloss）。
   - 用户可编辑生成结果；失败时提示并允许完全手动保存。
2) **单词卡片**
   - 双面翻转（点击/空格）；单列布局。
   - 正面：单词 + 罗马音；背面：单词、罗马音、释义、例句。
   - 扁平风格，高对比度，轻边框或浅投影。
3) **复习队列（30 张）**
   - 每日最多 30 张；优先抽取到期 SRS 卡片，不足时随机补足。
   - 操作：翻转；标记 Again/Hard/Good/Easy（快捷键 A/H/G/E）。
...
## 关键用户旅程
- **新增**：输入单词 → 生成 → AI 填充读音/释义/例句 → 用户微调 → 保存 → 卡片预览。
- **复习**：开始复习 → 正面 → 翻面 → 标记难度 → 下一张；结束展示正确率、下次到期预估。
...
## 数据模型（SQLite）
- `words`: id, term, reading_romaji, meaning, examples_json, created_at, updated_at, srs_level, due_at, tags
...
- `settings`: api_key, preferred_model, review_batch_size(默认 30), theme

## 复习 / SRS 逻辑
- 基于 SM-2：按 Again/Hard/Good/Easy 调整间隔，更新 `srs_level` 与 `due_at`。
- 队列生成：先取到期项按 `due_at` 排序；不足 30 时补充今日未见的随机词。
- 每次评分写入 `review_events`；总结页基于会话内聚合。

## AI 提示示例
- System: “你是一名简洁的日语导师。返回罗马音读音、带情景的简短释义，以及 1–2 组例句（JP + EN gloss）。”
- User: “Word: {term}. 若为专有名词/俚语请简述。”
- 校验：必须有罗马音；释义约 140 字符内；例句控制长度以便卡片展示。

## Heat Map 数据
- 来自 `daily_activity` 或对 `review_events`/`words` 按日聚合。
- 4–5 档颜色；方块留 2px 间距；顶部月份标签，悬停提示计数。
...
- 离线：复习完整可用；新增可离线保存原始词，联网后补充生成（状态标记）。
