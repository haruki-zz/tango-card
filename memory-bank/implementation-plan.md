...
17. UI 设计规范落地（主题与资源）
    - 步骤：将 UI 设计文档的色板、字体、间距、阴影与动效抽取为 Tailwind/全局 CSS 变量；用开源 Noto Serif JP 替代游明朝/教科書体并配置英文字体等宽回退；准备 Feather/Remix 16px 线框图标与自制轻噪点纸纹理；整理描边/悬浮/禁用等基础按钮与输入态。
    - 结果：形成统一可复用的主题 tokens 与基础样式，后续组件无需重复定义视觉细节。
    - 验证：`npm run dev` 查看全局背景、字体与按钮态颜色/字号符合设计规范，lint 通过。

18. 主界面布局与新增区重写
    - 步骤：重构 `App.tsx` 布局为 24%/46%/30% 三列并加入顶部工具区（设置/全屏/暗亮切换占位，不切换主题），使用虚线或浅线分割；左侧新增区按设计调整输入占位文案、生成/保存按钮描边与状态、生成结果卡片的 Noto Serif JP 字体与提示条（成功/错误浅橙背景）。
    - 结果：主界面骨架与新增流程的视觉层次、间距和提示样式符合设计，操作路径清晰。
    - 验证：在 ≥1200px 下手动确认列宽比例与分割线效果，执行 AddWordFlow 等相关测试确保生成/保存流程保持通过。

19. 复习卡片与交互强化
    - 步骤：更新复习卡片为立体描边风格，长词自动缩放；补齐 0–5 六个评分按钮、左右箭头切换、进度条+百分比显示；新建 CSS 动效实现 Y 轴 200–240ms 翻转与分割线描边效果，并完善键盘快捷键提示/响应。
    - 结果：复习流突出当前卡片与评分操作，交互与动画符合设计预期。
    - 验证：补充/运行 ReviewSession 测试覆盖进度计算、按钮态与键盘事件；手动翻转/评分确认动画、禁用态与导航无异常。

20. 右侧活跃度与数据面板重构
    - 步骤：调整热力图为 5x7（可扩展到 6 周）方格，直接用后端提供的 35 天数据按 0/1–3/4–6/7–14/15+ 绿色梯度映射并展示 tooltip；当日数据与今日新增/復習 session 分栏对齐；导入/导出描边按钮与结果提示；AI Provider 下拉与 API Key 输入展示已保存标签与遮蔽。
    - 结果：统计、导入导出与 provider 设置区符合设计的配色、对齐和提示交互，数据一屏可见。
    - 验证：更新 ActivityHeatmap/ImportExportPanel/SettingsPanel 测试覆盖色阶映射、tooltip/路径/标签提示；手动 hover/点击确认展示与状态切换。

21. 响应式与可访问性验收
    - 步骤：实现 ≥1200/960–1199/768–959/<768 四档响应式布局（含评分按钮双行、右侧堆叠、满宽容器）、Tab 顺序与占位/提示文案调整，补充 hover/focus/toast 的可达性与对比度检查。
