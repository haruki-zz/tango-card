...
## 阶段 2：数据层与 IPC
5. 建立 SQLite + Drizzle：定义 Word、ReviewSession、Activity 表 schema，配置迁移脚本与用户数据目录下的数据库初始化流程。  
   - 测试验证：运行迁移命令生成并应用迁移文件；Vitest 以临时数据库实例校验表结构、约束与默认值。
6. 数据访问封装：在主进程实现仓储函数（增词、随机取词、创建复习 session、写入活动统计、查询活跃度），封装错误处理与事务边界。  
...
## 阶段 3：AI 服务与新增单词（核心）
8. AI 调用模块：实现基于 fetch 的可插拔客户端，支持 API Key 环境变量、超时、最多 2 次重试、JSON 解析为 reading/meanings/examples 结构。  
...
## 阶段 4：复习队列与卡片（核心）
11. 复习队列逻辑：在 renderer 编写队列 store，调用 word:listRandom 获取 30 张唯一卡片，支持重新抽取与队列进度。  
    - 测试验证：Vitest 对队列生成函数进行随机性与唯一性检查，模拟少于 30 个词时的边界处理。
12. ReviewSession 记录：完成 review:create 调用以保存本次词 IDs、开始/完成时间与掌握标记；处理失败时重试或缓存。  
...
15. 活跃度聚合查询：在主进程实现最近 12 周的按日聚合（words_added、reviews_completed），返回适配 Heat Map 的数据结构。  
    - 测试验证：数据库单测构造多日数据，断言聚合结果与日期排序正确，缺失日期填零。
16. Heat Map 组件：在 renderer 绘制格子矩阵，支持 hover 提示与点击弹出该日详情（显示新增/复习数量）。  
