...
16. 主页聚合与导航入口  
    - 在首页聚合关键功能：主卡片随机展示词库（优先 unfamiliar 且复习次数最高，复习次数相同随机，其次熟悉词，词库为空显示提示卡），提供新增单词按钮、复习入口（挂载 `ReviewSession` 路由）、下方内嵌当前月 Heat Map 预览与当月新增/复习计数摘要，点击 Heat Map 跳转完整页。  
    - 加入本地数据库加载态与错误提示，确保首页在 DB 未就绪时也有用户反馈。  
    - 测试：RNTL 场景测试验证抽卡优先级/空态、导航跳转（新增/复习/Heat Map）、当月统计范围正确，DB 异常时提示渲染。
