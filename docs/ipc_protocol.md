...
| --- | --- | --- | --- | --- |
| `card:ingest` | Renderer → Main | `{ card_id?: string, word: string, reading: string, context: string, scene: string, example: string, created_at?: string }` | `CardEntity` | 创建或更新单词卡。主进程会根据是否携带 `card_id` 调用对应的持久化逻辑。 |
