...
## 主界面布局（竖屏优先）
主界面按照移动端竖屏逻辑重新规划为三段式结构，便于后续在窄屏设备上复用：
- **学习进度卡片**：页面顶部展示 `<ContributionHeatmap>` 的压缩版摘要卡片，使用圆角描边与渐隐投影，点击后进入完整的统计面板。
- **单词卡片概览**：居中区域显示 `<WordsCardSummary>`（将在渲染层封装）的大卡片，提供待复习数量、最近新增等摘要信息，后续可跳转到卡片管理视图。
- **底部操作区**：页面底部固定一个主行动按钮“Start Review”，采用胶囊按钮与浅色背景凸显；右下角悬浮圆形 “➕” 按钮触发卡片创建流程，对应现有 `CardEditorScreen`。

整体容器采用竖直堆叠与安全区域内边距（Tailwind `flex-col`、`gap`），在桌面端保持居中留白，在移动端占满宽度；所有卡片宽度统一，使用阴影/描边区分层次。该设计保证桌面端与未来移动端共用一套组件，优先满足竖屏交互体验。
