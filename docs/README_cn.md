...
1. **创建单词卡**：输入单词、平假名读音、语境与例句，模板会将四项信息排版到统一尺寸的 SVG 中；保存时可选择记忆等级。
2. **复习单词卡**：按记忆等级权重抽取待复习的卡片，使用数字/方向键或滑动操作快速记录记忆程度。
...
- **记忆等级策略**：`src/domain/review` 中的策略根据 `MemoryLevel`（熟知 / 一般 / 需巩固）动态调整抽取概率，低熟悉度的卡片更容易被选中。
...
- **编辑界面**：左侧为表单（单词、平假名、上下文、例句、记忆等级），右侧为实时 SVG 预览。
- **复习界面**：上方提供“Start review” 按钮及队列说明；下方分为卡片展示区与记忆等级选择区，强调最少信息、最少动作。
