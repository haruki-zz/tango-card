...
## 主界面布局（竖屏优先）
主界面遵循 UI 目录下 `UI/main-interface.svg` 的设计稿，延续移动端竖屏的三段式结构：
- **学习热力图区**：顶部 `HomeHeatmapSection` 展示每日活跃热力图，可在“综合活动 / 新增单词 / 复习次数”间切换，并附带连续打卡天数与近 7 日统计，点击“View Details” 跳转至统计页。
- **精选单词卡**：中部 `HomeFeaturedCard` 随机抽取一张已存在单词卡，完整呈现“单词 / 平假名读音 / 语境 / 场景 / 例句”五项内容；无数据时提示用户先创建卡片。
- **底部操作区**：保留一体化的“Start Review” 胶囊按钮与“➕” 圆形按钮，分别触发新一轮复习和创建单词流程，对应 `review_screen` 与 `card_editor_screen`。

容器侧继续使用安全区域内边距与统一宽度的卡片布局（Tailwind `flex-col`、`gap` 样式），通过渐进阴影与毛玻璃背景区分层级，以便桌面端和窄屏设备复用。
