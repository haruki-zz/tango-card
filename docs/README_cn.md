...
除这两个流程外，其余分析类面板、备份工具、复杂仪表盘均已移除，确保应用保持轻量；界面支持明/暗两种主题，随时切换。

## 核心能力
- **统一的 SVG 模板**：`src/shared/templates/card_svg_template.ts` 将四项输入渲染为固定画布，保证复习界面始终使用一致的尺寸与排版。
- **复习策略**：`src/domain/review` 中的策略按熟悉度加权生成队列（默认偏向“不太熟悉”单词），并记录复习次数与最后复习时间。
- **复习交互**：复习界面支持点击按钮、键盘（←/→，Enter/Shift+Enter 入口处跳转）、滑动手势，方便在桌面设备或触控板上快速录入；复习时可以为单词添加/移除“不太熟悉”标签，突出薄弱词汇。
- **活跃度热力图**：主页底部提供全年（365 天）GitHub 风格的 Heat Map，单元格 15px、间距 5px，周一在第一行，月份按列分段并用空列分隔，零活跃度为白色、活跃度越高绿阶越深（主题自适应）。数据随新增/复习事件实时更新，并随窗口宽度自动拉伸。
- **主题切换**：顶栏提供 Light/Dark 切换，卡片 SVG、热力图、界面元素均使用 CSS 变量自动适配主题。
- **本地持久化**：Electron 主进程通过文件存储的仓库 (`CardRepository` / `ReviewSessionRepository`) 同步数据，渲染层使用 IPC 调用 API。

## 界面结构
- **入口界面**：中心卡片内展示两枚 CTA（创建/复习）与一份状态板，底部全年热力图占据剩余空间；支持 Enter 创建、Shift+Enter 复习的快捷键。
